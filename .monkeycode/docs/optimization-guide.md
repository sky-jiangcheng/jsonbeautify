...
### 7.8 iOS App Store：图标仍为 Tauri 默认图标

**现象**：iOS 应用上架 App Store 后，桌面/设置中的应用图标仍为 Tauri 默认图标（蓝色圆形），而非项目的自定义图标。

**根因（三重叠加）**：

1. **`tauri.ios.conf.json` 没有 `bundle.icon`**：仅有 `identifier` 字段，Tauri 回退到主 `tauri.conf.json` 的 `bundle.icon`，而主配置只有桌面图标尺寸，不含 iOS 规格的图标路径。

2. **CI iOS job 缺 `npx tauri icon`**：release.yml 中 macOS job 有 `npx tauri icon src-tauri/icons/icon.png` 步骤，iOS job 没有。`tauri ios init` 生成 Xcode 项目时，`Assets.xcassets/AppIcon.appiconset/` 需要完整的 iOS 图标资源，缺少此步骤时使用内置默认图标。

3. **`generate-icons.js` 只生成桌面图标**：脚本仅覆盖 16-512px 桌面尺寸和 PWA 图标，不含 iOS 的 20/29/40/60/76/83.5/1024 多种尺寸。

**修复**：

1. **`tauri.ios.conf.json` 添加 `bundle.icon`**，包含所有 iOS 图标路径：
...
```json
{
  ...
  "bundle": {
    "icon": [
      ...
      "icons/16x16.png", "icons/64x64.png", "icons/256x256.png",
      "icons/512x512.png", "icons/512x512@2x.png",
      "icons/ios/AppIcon-20x20@1x.png", "icons/ios/AppIcon-20x20@2x.png",
      "icons/ios/AppIcon-20x20@3x.png", "icons/ios/AppIcon-29x29@1x.png",
      "icons/ios/AppIcon-29x29@2x.png", "icons/ios/AppIcon-29x29@3x.png",
      "icons/ios/AppIcon-40x40@1x.png", "icons/ios/AppIcon-40x40@2x.png",
      "icons/ios/AppIcon-40x40@3x.png", "icons/ios/AppIcon-60x60@2x.png",
      "icons/ios/AppIcon-60x60@3x.png", "icons/ios/AppIcon-76x76@1x.png",
      "icons/ios/AppIcon-76x76@2x.png", "icons/ios/AppIcon-83.5x83.5@2x.png",
      "icons/ios/AppIcon-1024.png"
    ]
  }
}
```

2. **CI release.yml iOS job 加图标生成步骤**（在 `tauri ios init` 之前）：

```yaml
- name: Generate iOS icons
  run: npx tauri icon src-tauri/app-icon-source.png   # ⚠️ 必须用权威源 app-icon-source.png，绝不能用 icons/icon.png（那是 tauri icon 的输出文件，会被自身覆盖）

- name: Initialize iOS project
  run: npx tauri ios init --ci --config src-tauri/tauri.ios.conf.json
```

3. **`generate-icons.js` 扩展 iOS 尺寸**，添加 12 种 iOS App Icon 规格（20pt 1x/2x/3x, 29pt 1x/2x/3x, 40pt 1x/2x/3x, 60pt 2x/3x, 76pt 1x/2x, 83.5pt 2x, 1024x1024）。

**教训**：iOS 图标生成步骤必须在 `tauri ios init` 之前执行，因为 `init` 会从 `bundle.icon` 列表和实际文件重新生成 Xcode 的图标资源目录。两者缺一不可——配置文件声明路径 + 对应的 PNG 文件物理存在。

#### 🔴 2026-07-20 深层根因补充（修了三轮才坐实）

上面「三重叠加」是 v1.5.9~1.5.10 的理解，**仍未触底**。1.5.11 TestFlight 列表仍显示默认环，像素级复盘后定位到**两个更深的坑**：

1. **CI 的 `tauri icon` 源用错了文件**：`npx tauri icon src-tauri/icons/icon.png` —— `icon.png` 是 `tauri icon` **自己的输出文件**！commit `dbcf9bc` 专门建 `app-icon-source.png` 就是「**避免被 tauri icon 覆盖**」作权威源，但 CI 没用它、仍拿输出当源。本次 `icon.png` 恰好也是蓝J，属「用错但运气好」。**正确：源必须是 `app-icon-source.png`**（已修，commit `13b6bb4`）。

2. **TestFlight/ASC 列表图标 ≠ build 内嵌图标**：TestFlight「构建版本」列表显示的是 **App 在 ASC 里登记的 app-level icon**（从建 app 起就没换过的默认环），不是每个 build 自己的图标。即使 build 里是蓝J，列表也显环。**修法：在 ASC 后台 → App Information（或版本 App Store 标签 → Media → App Icon）上传一张 1024×1024 蓝J**（仓库 `src-tauri/icons/ios/AppIcon-1024.png` 即可，alpha 全 255 合规）。

3. **判断图标真伪别用「看着像」**：连续多轮「确认是蓝J」实为基于摘要/截图的臆测。**ground truth = 程序读 PNG 像素渲染成 ASCII**（如 `python3 /tmp/png_ascii.py <file>` 输出 `@`=黑底 `B`=蓝J），辅以 MD5 + 构建链分析。模型读图不可靠。

---

### 7.9 提交审核报 403：REJECTED 版本残留 appStoreVersionSubmission
...
**修复**（见 `submit_for_review`，commit `8083947`）：提交前先 `GET /v1/appStoreVersions/{id}/appStoreVersionSubmission`，若存在则 `DELETE /v1/appStoreVersionSubmissions/{id}`（版本回到可编辑），再 POST 创建新 submission；403 时带退避重试。依据 Apple 文档 `DELETE` 返回 204。

> 关键好消息：此 403 发生在「关联构建」**之后**，意味着 build 已上传+挂到版本，最耗时的环节已完成，只是最后「点提交」姿势错。**不必重跑 CI 重建**，补完资料后 ASC UI 手动 Submit 即可。

---

### 7.10 脚本把「构建号」当「商店版本号」查 → 永远 409

**症状**：CI 长期每次提交都 409 `ENTITY_ERROR.RELATIONSHIP.INVALID`，只能手动在 ASC 挂 build。

**根因**：`submit-appstore-review.py` 旧逻辑用 `APP_VERSION`（**构建号**，如 `1.5.11`）去查 App Store 的 `versionString`，而商店版本是 **marketing version**（如 `1.0`）→ 永远查不到 → 每次都 POST 创建新版本 → 永远 409（详见 §6.5）。
...
### 7.12 iOS PWA standalone 模式下页面整体模糊

**现象**：将站点添加到 iPhone 主屏幕后，以 PWA standalone 模式打开，整个页面（文字、UI 元素）呈现轻微模糊，类似低分辨率渲染后放大的效果。Safari 浏览器中直接打开则正常。

**根因（三重叠加）**：

1. **viewport 缺 `minimum-scale=1.0` 和 `shrink-to-fit=no`**：iOS PWA standalone 模式使用 WKWebView 渲染，无 Safari 浏览器 chrome。此时 viewport 计算与浏览器模式不同，iOS 可能以低于设备原生分辨率渲染页面再缩放，导致整体模糊。

2. **`-webkit-overflow-scrolling: touch`**：该属性强制创建 GPU 合成层，在 iOS 上已知会导致该层内文字渲染发虚。移动端工具栏和侧边栏列表均使用了此属性。iOS 13+ 原生 `overflow: auto` 已支持平滑滚动，不再需要此 hack。

3. **`body { -webkit-font-smoothing: antialiased }`**：关闭亚像素抗锯齿，使用灰度抗锯齿。iOS 设备默认已处理好字体渲染，显式覆盖反而可能劣化效果。

**修复**：

1. viewport meta 添加 `minimum-scale=1.0, shrink-to-fit=no`：
```html
<meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, viewport-fit=cover, shrink-to-fit=no">
```

2. 移除所有 `-webkit-overflow-scrolling: touch`（`.mob-toolbar`、`.history-list` 等移动端滚动容器）

3. `body` 用标准 `text-rendering: optimizeLegibility` 替换 vendor-prefixed font-smoothing：
```css
body {
...
**教训**：
- iOS PWA standalone 模式的 WKWebView 与 Safari 浏览器不是同一个渲染上下文，viewport 行为有差异
- `-webkit-overflow-scrolling: touch` 是 iOS 遗留 hack，iOS 13+ 应移除
- `-webkit-font-smoothing: antialiased` 在移动端往往不如默认的 subpixel-antialiased，不要全局覆盖
...
- [ ] iOS CI job 有 `npx tauri icon` 步骤，`tauri.ios.conf.json` 含 `bundle.icon`，**且 icon 源是 `app-icon-source.png`（绝非 `icons/icon.png` 输出文件）**
- [ ] **ASC 后台已上传 App 级 1024×1024 图标**（TestFlight/商店列表显示的是 app-level icon，非 build 内嵌图标；不上传则永远显默认环）
- [ ] `submit-appstore-review.py` 提交前先 `GET`+`DELETE` 残留 `appStoreVersionSubmission`（防 REJECTED/DEVELOPER_REJECTED 版本 403）
- [ ] 脚本按 **marketing version**（如 1.0）选提交版本，**构建号**（如 1.5.11）仅用于匹配 VALID build（防 409）
- [ ] 判断图标真伪用**像素解码**（程序读 PNG 渲染 ASCII），不用「看着像」/模型读图
- [ ] 移动端 viewport 含 `minimum-scale=1.0, shrink-to-fit=no`，不含 `user-scalable=no`
